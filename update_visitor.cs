...
namespace sarasavi_library_management_system
{
    public partial class update_visitor : Form
    {
        ...
        {
            // edit table data
            string SQL = "UPDATE library_visitor SET " +
                "visitor_number= '" + textBox1.Text + "', " +
            ...
            //  MessageBox.Show(responce);
            {
                ...
                if
                        (responce == "Success")
                {
                    MessageBox.Show("Records Updated Successfull", "SARASAVI LIBRARY::Update Visitor Details", MessageBoxButtons.OK, MessageBoxIcon.Information);

                }
                else
                {
                    MessageBox.Show("Records Updated Failed!", "SARASAVI LIBRARY::Update Visitor Details", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
                return;
            }
        ...
        {
            {
                if (textBox6.Text == "")
                {
                    MessageBox.Show("Enter Search Id", "SARASAVI LIBRARY::Update Visitor Details", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    textBox6.Focus();
                    return;
                }
                else
                {
                    ...
                    DataRow ReadData = conf.FetchRec("library_visitor", "Id", textBox6.Text);
